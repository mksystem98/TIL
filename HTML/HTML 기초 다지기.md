...
> [!note] Visual Studio Code에서 ' ! + tab ' 을 통해 간단하게 HTML 상용구를 생성할 수 있다.
...
> [!tip] 표준 HTML 문서 구조는 하나의 `<html>` 요소 내에  `<head>` 섹션 하나와 `<body>` 섹션 한 쌍으로 이루어져야 한다.
